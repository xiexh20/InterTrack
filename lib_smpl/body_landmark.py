...
class BodyLandmarks:
    ...
    def __init__(self, assets_root, bs=1, device='cpu'):
        ...
        # cache to compute body joints
        body25_reg, face_reg, hand_reg = load_regressors(assets_root, bs)
        self.body25_reg_th = body25_reg.to(device)
    ...
    def get_body_kpts_th(self, smpl_verts):
        """
        smpl_verts is torch tensor, probably in cuda
        return (B, J, 3)
        """
        from .torch_functions import batch_sparse_dense_matmul
        J = batch_sparse_dense_matmul(self.body25_reg_th, smpl_verts)
        return J
